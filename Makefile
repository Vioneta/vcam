...
VERSION = 0.3.0
IMAGE_REPO ?= ghcr.io/vioneta/vioneta-vcam
...
local: version
	docker buildx build --target=frigate --tag vioneta-vcam:latest --load --file docker/main/Dockerfile .
...
run: local
	docker run --rm --publish=5000:5000 --volume=${PWD}/config:/config vioneta-vcam:latest

run_tests: local
	docker run --rm --workdir=/opt/frigate --entrypoint= vioneta-vcam:latest python3 -u -m unittest
	docker run --rm --workdir=/opt/frigate --entrypoint= vioneta-vcam:latest python3 -u -m mypy --config-file frigate/mypy.ini frigate
